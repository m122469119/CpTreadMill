...
class BannerPagerAdapter(var context: Context): InfinitePagerAdapter(), IconPagerAdapter {
    ...
    override fun getView(position: Int, convertView: View?, container: ViewGroup?): View {
        ...
        if (rootView == null) {
            rootView = LayoutInflater.from(context).inflate(R.layout.item_banner_img, container, false)
            holder = ViewHolder(rootView)
            rootView.tag = holder
        } else {
            holder = rootView.tag as ViewHolder
        }
        holder.bindView(context, mBannerList[position])
        return rootView!!
    ...
    class ViewHolder(var view: View) {
        ...
        fun bindView(context: Context, banner : String){
